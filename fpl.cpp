...
public:
    FantasyPremierLeague() : budget(TOTALBUD), botBudget(TOTALBUD) {
        //player limits for squad in each position
    ...
    void draftSquad(vector<Player>& squad, double& remainingBudget, bool isBot = false) {
        ...
        sort(available.begin(), available.end(), compareByPrice);
        ...
        for (auto i = available.begin(); i != available.end();) {
            if (positionCount[i->position] < positionLimits[i->position] && remainingBudget >= i->price) {
                squad.push_back(*i);
                ...
                i = available.erase(i);  // Remove from availablePlayers
    ...
    void loadPlayers() {
            ...
            file.close();
            cout << "Players loaded from CSV successfully!\n";
        }
...
//                    cout << "Season over! Final score: " << totalScore << " points.\n";
//                    cout << "Bot's final score: " << botScore << " points.\n";
//                    if (totalScore > botScore)
//                        cout << "Congratulations! You defeated the bot!\n";
//                    else if (totalScore < botScore)
